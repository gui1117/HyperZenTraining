...
impl<'a> ::specs::System<'a> for HookSystem {
    ...
    fn run(
        &mut self,
        (bodies, aims, mut hooks, physic_world, entities): Self::SystemData,
){
        for (aim, body, hook, entity) in (&aims, &bodies, &mut hooks, &*entities).join() {
            // Delete anchor if entity doesn't exist anymore
            if let Some(false) = hook.anchor.as_ref().map(|anchor| entities.is_alive(anchor.entity)) {
                hook.anchor = None;
                hook.launch = false;
            }
            ...
            if hook.launch && hook.anchor.is_none() {
                ...
                for collided in &self.collided {
                    let other_pos = bodies.get(collided.0).unwrap().get(&physic_world).position();
            ...
            if let Some(ref mut anchor) = hook.anchor {
                anchor.pos = (bodies.get(anchor.entity).unwrap().get(&physic_world).position() * anchor.local_pos).coords;
            }
        }
    }
}
